...
export default function Home({ user, onLogout, onOpenMod, onGoAdmin }) {
  ...
  return (
    <>
        ...
        <div className="nav-links">
          {['cap','rec','exa','ava'].map((t, i) => (
            <button key={t} className={'nb' + (tab === t ? ' active' : '')} onClick={() => setTab(t)}>
              ...
              {t === 'exa' && <span className="nbadge">{examenes.filter(e => !e.archivado && e.estado === 'activo').length}</span>}
            </button>
          ))}
        </div>
      ...
      <div className="hero" style={{ height: tab === 'cap' ? '230px' : '180px' }}>
        ...
        <div className="hero-c">
          {tab === 'cap' && (
            <>
              ...
              <div style={{ display:'flex', gap:0 }}>
                {[
                  { n: progreso.length ? Math.round(progreso.reduce((a,b) => a + b.porcentaje, 0) / progreso.length) : 0, l:'Tu progreso', s:'%' },
                  { n: modulos.length, l:'Módulos', s:'.' },
                  { n: 29, l:'Competencias', s:'.' },
                ].map((s, i) => (
                  <div key={i} style={{ paddingRight: i < 2 ? '1.2rem' : 0, borderRight: i < 2 ? '1px solid rgba(255,255,255,.14)' : 'none', marginRight: i < 2 ? '1.2rem' : 0 }}>
                    <div style={{ fontSize:'18px', fontWeight:800, color:'#fff', lineHeight:1 }}>{s.n}<span style={{ color:'var(--red)' }}>{s.s}</span></div>
          ...
          {tab === 'rec' && <><div className="hero-ey">Biblioteca técnica</div><h1>Recursos y Material</h1></>}
          {tab === 'exa' && <><div className="hero-ey">Evaluaciones pendientes</div><h1>Exámenes</h1></>}
          {tab === 'ava' && <><div className="hero-ey">Seguimiento personal</div><h1>Mi Avance</h1></>}
        </div>
      </div>
          ...
          <div className="sec">
            ...
            {loading ? (
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(170px,1fr))', gap:'1px', background:'var(--g100)', border:'1px solid var(--g100)' }}>
                      ...
                      <div className="mcard-body">
                        ...
                        <div>
                          ...
                          <div style={{ display:'flex', justifyContent:'space-between', marginTop:'4px' }}>
                            ...
                            <span style={{ fontSize:'8.5px', color:'rgba(255,255,255,.38)' }}>{(m.lecciones || []).length} lecciones</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                {modulos.length === 0 && (
                  <div style={{ gridColumn:'1/-1', padding:'2rem', textAlign:'center', color:'var(--g400)', fontSize:'11px' }}>
      ...
      {tab === 'rec' && (
        <div className="sec">
          <div className="sec-lbl" style={{ marginBottom:'.8rem' }}>Documentos y videos disponibles</div>
          ...
          {material.filter(m => !m.archivado && m.estado === 'activo').length === 0 && (
            <div style={{ fontSize:'11px', color:'var(--g400)', padding:'.5rem 0' }}>No hay material disponible aún.</div>
          )}
        </div>
      )}
      ...
      {tab === 'exa' && (
        <div className="sec">
          ...
          {examenes.filter(e => !e.archivado && e.estado === 'activo').length === 0 && (
            <div style={{ fontSize:'11px', color:'var(--g400)', padding:'.5rem 0' }}>No hay exámenes disponibles aún.</div>
          )}
        </div>
      )}
      ...
      {tab === 'ava' && (
        <div className="sec">
            ...
            return (
              <div key={m.id} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'.7rem 0', borderBottom:'1px solid var(--g100)' }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ height:'3px', background:'var(--g100)' }}>
                    <div style={{ height:'100%', width: p + '%', background:'var(--red)' }} />
                  </div>
                </div>
          ...
          {modulos.length === 0 && (
            <div style={{ fontSize:'11px', color:'var(--g400)', padding:'.5rem 0' }}>No hay módulos disponibles aún.</div>
          )}
        </div>
      )}
    </>
